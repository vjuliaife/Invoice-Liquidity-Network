...
import { GovernanceClient, GOVERNANCE_TESTNET, ProposalActionKind, ProposalStatus, parseGovernanceProposalSimulation, parseGovernanceProposalListSimulation, } from "../src/governance";
...
function createMockServer(accountAddress) {
    return {
        getAccount: vi.fn().mockResolvedValue(new Account(accountAddress ?? Keypair.random().publicKey(), "12")),
...
function sampleProposalNative(overrides = {}) {
    return {
        id: 1n,
...
describe("GovernanceClient", () => {
    it("builds createProposal transaction with correct contract call", async () => {
        ...
        expect(Buffer.from(scValToNative(call.args[2]))).toEqual(DESCRIPTION_HASH);
    ...
    it("builds executeProposal transaction with correct contract call", async () => {
        ...
        expect(scValToNative(call.args[1])).toBe(20000n);
    });
    it("builds vetoProposal transaction with correct contract call", async () => {
        ...
        expect(Buffer.from(scValToNative(call.args[1]))).toEqual(REASON_HASH);
    });
    ...
    it("parses getProposal simulation into typed proposal data", async () => {
        ...
        const proposal = parseGovernanceProposalSimulation(simulation, GovernanceContractMethod.GetProposal);
    ...
    it("parses listProposals simulation into typed proposal list", async () => {
        ...
        const proposals = parseGovernanceProposalListSimulation(simulation, GovernanceContractMethod.ListProposals);
        expect(proposals).toHaveLength(2);
        expect(proposals[0]?.id).toBe(1n);
        expect(proposals[1]?.id).toBe(2n);
    });
    it("propagates simulation errors from getProposal parsing", () => {
        expect(() => parseGovernanceProposalSimulation({ error: "contract reverted" }, GovernanceContractMethod.GetProposal)).toThrow("Simulation failed for get_proposal: contract reverted");
    });
    it("propagates contract errors from getProposal parsing", () => {
        expect(() => parseGovernanceProposalSimulation({
            result: {
                retval: nativeToScVal({ err: "ProposalNotFound" }),
            },
        }, GovernanceContractMethod.GetProposal)).toThrow("Contract method get_proposal returned an error: ProposalNotFound.");
    });
    it("propagates contract errors from listProposals parsing", () => {
        expect(() => parseGovernanceProposalListSimulation({
            result: {
                retval: nativeToScVal({ Err: 2 }),
            },
        }, GovernanceContractMethod.ListProposals)).toThrow("Contract method list_proposals returned an error: 2.");
    });
    it("rejects invalid description hash length in parser", () => {
        expect(() => parseGovernanceProposalSimulation({
            result: {
                retval: nativeToScVal(sampleProposalNative({ description_hash: Buffer.alloc(16, 0x01) })),
            },
        }, GovernanceContractMethod.GetProposal)).toThrow("Expected description hash to be 32 bytes.");
    });
...
describe("governance encoding helpers", () => {
    it("rejects non-32-byte hash values", async () => {
        ...
        const server = createMockServer(proposer);
        const client = createClient(server);
        await expect(client.createProposal({
            proposer,
