...
function createSdk(server, signer) {
    return new ILNSdk({
...
describe("ILNSdk", () => {
    ...
    it("rejects a batch with more than 100 operations", async () => {
        ...
        const sdk = createSdk(server);
        const operations = Array.from({ length: 101 }, () => Operation.invokeContractFunction({
            source,
            contract: CONTRACT_ID,
            function: "mark_paid",
            args: [nativeToScVal(1n, { type: "u64" })],
        }));
        await expect(sdk.batch(operations)).rejects.toThrow("Batch cannot contain more than 100 operations.");
    });
    ...
    it("throws when a transaction signer is required but not provided", async () => {
        ...
        await expect(sdk.markPaid({ invoiceId: 9n })).rejects.toThrow("A transaction signer is required for state-changing contract calls.");
    });
    it("throws when simulation fails with an error", async () => {
        ...
        const sdk = createSdk(server);
        await expect(sdk.getInvoice(1n)).rejects.toThrow("Simulation failed for get_invoice: Some RPC failure");
    });
    it("throws when sendTransaction returns an invalid response", async () => {
        ...
        const sdk = createSdk(server, signer);
        await expect(sdk.markPaid({ invoiceId: 9n })).rejects.toThrow("RPC server returned an invalid sendTransaction response.");
    });
    it("throws when contract result is an Err", async () => {
        ...
        const sdk = createSdk(server);
        await expect(sdk.getInvoice(1n)).rejects.toThrow("Contract method get_invoice returned an error: Invalid something.");
    });
    it("rejects submitInvoice when the provided freelancer does not match the signer", async () => {
        const signer = createKeypairSigner(Keypair.random().secret());
        const sdk = createSdk({}, signer);
        await expect(sdk.submitInvoice({
