 ...
 */
export async function checkCompatibility(invoke) {
    const issues = [];
    ...
    const isOlderThanMin = cMajor < mMajor ||
        (cMajor === mMajor && cMinor < mMinor) ||
        (cMajor === mMajor && cMinor === mMinor && cPatch < mPatch);
    if (isOlderThanMin) {
        issues.push(`Deployed contract version (${contractVersion}) is older than the minimum required version (${MIN_CONTRACT_VERSION}) supported by this SDK.`);
    }
    // Detect potentially breaking contract major version mismatch
    if (cMajor > sMajor) {
        issues.push(`Deployed contract version (${contractVersion}) has a higher major version than the SDK (${SDK_VERSION}), which may introduce breaking changes.`);
    }
    return {
