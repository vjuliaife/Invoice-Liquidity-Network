...
function parseProposalAction(value) {
...
function parseProposalActionFromTag(tag, payload) {
    switch (tag) {
...
export function parseGovernanceProposal(value) {
    const proposal = value;
    const descriptionHash = toBuffer(proposal.description_hash ?? proposal.descriptionHash, "descriptionHash");
...
export function parseGovernanceProposalSimulation(simulation, method) {
...
export function parseGovernanceProposalListSimulation(simulation, method) {
    const retval = extractSimulationRetval(simulation, method);
    const native = unwrapContractResult(scValToNative(retval), method);
